...
class BlockNum(pygame.sprite.Sprite):
    ...
    def click(self, rightnum):
        ...
        test_rect = pygame.Rect(
            (0, 0), (85, 85)
        )  # 指定大小，格子大小为110＊110（含线宽）,字体大小为80，比字体稍大，不大于格子
...
def main():
    ...
    # 历史成绩按钮及保存按钮
    bt_font_size = 50
    history_surf = blitfont(
        "历史成绩", font=ftpath, ftsize=bt_font_size, bgcolor="blue"
    )
    ...
    save_rect = history_surf.get_rect(
        right=screen.get_rect().right - 30, y=screen.get_height() - 100
    )
            ...
            if e.type == pygame.MOUSEBUTTONDOWN:
                for item in numsp:
                    if item.click(cur_num):
                        ...
                        break
                if (
                    save_rect.collidepoint(pygame.mouse.get_pos())
                    and cur_num == 0
                    and history_tag == False
                    and saved == False
                ):
                    with shelve.open(score_file, writeback=True) as f:
                        f_score = f.get("highest_score")
                        if f_score:
                            f_score.append(cur_time)
                            f_score.sort()
                            if len(f_score) > 5:
                                f_score.pop()
                        else:
                ...
                if (
                    history_rect.collidepoint(pygame.mouse.get_pos())
                    and cur_num == 0
                    and history_tag == False
                ):
                    ...
                    while i < 5:
                        try:
                            ...
                            score_rect = score_surf.get_rect(
                                centerx=screen.get_rect().centerx, y=100 + i * 80
                            )
                    ...
                    history_tag = True
                if history_tag == True and back_rect.collidepoint(
